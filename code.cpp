...
int menu() {//Меню в виде цикла
    ...
    else
    {
        ...
        case 2:
        {
            std::cout << "Капитал-шоу “Поле чудес” – советская и российская телеигра, российский аналог американской телевизионной программы “Колесо Фортуны”. Художественный руководитель и ведущий программы – Леонид Якубович. Производитель — Телекомпания “ВИD”. Загадывается слово, твоя задача угадать его последовательно по буквам. Важно: могут быть ответы, загаданные из более чем одного слова, их следует писать слитно! У тебя будет описание слова и пара бонусов. На всю игру дается 10 попыток" << std::endl;
            return 2;
        }
        case 3:
        {
...
std::string strcinema(int random) {
    std::string text[] = {
        ...
        "Мультфильм про рыбу-клоуна, ищущую своего сына",
        "Герой комиксов, который носит маску и сражается с преступностью ночью.",
        "Мультфильм про игрушечного ковбоя и космонавта."
    };
    std::cout << text[random] << std::endl;

    std::string word[] = { "пеппа", "гаррипоттер", "рапунцель", "нэмо", "бэтмен", "историяигрушек" };
    return word[random];
}
...
std::string strtechnology(int random) {
    std::string text[] = {
        ...
        "Первая в мире операционная система с графическим интерфейсом.",
        "Социальная сеть, где публикуют короткие видео.",
        "Электронное устройство для хранения и обработки информации."
    };
    std::cout << text[random] << std::endl;

    std::string word[] = { "яндекс", "принтер", "айфон", "макинтош", "тикток", "компьютер" };
    return word[random];
}
